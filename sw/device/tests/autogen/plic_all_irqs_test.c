...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
static volatile dif_iopmp_irq_t iopmp_irq_expected;
static volatile dif_iopmp_irq_t iopmp_irq_serviced;
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
      case kTopEarlgreyPlicPeripheralIopmp: {
        dif_iopmp_irq_t irq = (dif_iopmp_irq_t)(
            plic_irq_id -
            (dif_rv_plic_irq_id_t)kTopEarlgreyPlicIrqIdIopmpAccessViolation);
        CHECK(irq == iopmp_irq_expected,
              "Incorrect iopmp IRQ triggered: exp = %d, obs = %d",
              iopmp_irq_expected, irq);
        iopmp_irq_serviced = irq;

        dif_iopmp_irq_state_snapshot_t snapshot;
        CHECK_DIF_OK(dif_iopmp_irq_get_state(&iopmp, &snapshot));
        CHECK(snapshot == (dif_iopmp_irq_state_snapshot_t)(1 << irq),
              "Only iopmp IRQ %d expected to fire. Actual interrupt "
              "status = %x", irq, snapshot);

        CHECK_DIF_OK(dif_iopmp_irq_acknowledge(&iopmp, irq));
        break;
      }
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_EARLGREY_IOPMP_CFG_BASE_ADDR);
  CHECK_DIF_OK(dif_iopmp_init(base_addr, &iopmp));
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  CHECK_DIF_OK(dif_iopmp_irq_acknowledge_all(&iopmp));
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  dif_iopmp_irq_state_snapshot_t iopmp_irqs =
      (dif_iopmp_irq_state_snapshot_t)0xffffffff;
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  CHECK_DIF_OK(
      dif_iopmp_irq_restore_all(&iopmp, &iopmp_irqs));
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEarlgreyPlicPeripheralIopmp;
  for (dif_iopmp_irq_t irq = kDifIopmpIrqAccessViolation;
       irq <= kDifIopmpIrqAccessViolation; ++irq) {

    iopmp_irq_expected = irq;
    LOG_INFO("Triggering iopmp IRQ %d.", irq);
    CHECK_DIF_OK(dif_iopmp_irq_force(&iopmp, irq, true));

    // This avoids a race where *irq_serviced is read before
    // entering the ISR.
    IBEX_SPIN_FOR(iopmp_irq_serviced == irq, 1);
    LOG_INFO("IRQ %d from iopmp is serviced.", irq);
  }
#endif
